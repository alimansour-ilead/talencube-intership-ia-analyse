...
# ← models/vit_emotion.onnx n'est PAS généré ici (à la construction)
# — confirmé en production que ça échouait systématiquement (réseau
# restreint vers HuggingFace pendant le build sur Railway).
#
# Stratégie actuelle : ce fichier est généré automatiquement AU
# DÉMARRAGE DU SERVEUR (voir main.py) si absent — le réseau à
# l'exécution est souvent moins restreint que pendant la
# construction. Rien à faire ici dans le Dockerfile pour ça ;
# "COPY . ." copie déjà normalement export_onnx.py comme tout le
# reste du code, et main.py s'occupe de l'exécuter si besoin au
# démarrage.
